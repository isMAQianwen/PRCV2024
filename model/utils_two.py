...
class TrainSetLoader(Dataset):
    ...
    def __init__(self, dataset_dir, img_id ,base_size=512,crop_size=480,transform=None,suffix='.png'):
        ...
        self.images = dataset_dir+'/'+'image_raw'
        self.images_reverse = dataset_dir+'/'+'images'
        self.base_size = base_size
        self.crop_size = crop_size
        self.suffix = suffix

    def _sync_transform(self, img, imgreverse, mask):
        # random mirror
        if random.random() < 0.5:
            img = img.transpose(Image.FLIP_LEFT_RIGHT)
            imgreverse = img.transpose(Image.FLIP_LEFT_RIGHT)
        ...
        imgreverse = imgreverse.resize((ow, oh), Image.BILINEAR)
        ...
        if short_size < crop_size:
            ...
            imgreverse = ImageOps.expand(imgreverse, border=(0, 0, padw, padh), fill=0)
        ...
        imgreverse = imgreverse.crop((x1, y1, x1 + crop_size, y1 + crop_size))
        mask = mask.crop((x1, y1, x1 + crop_size, y1 + crop_size))
        # gaussian blur as in PSP
        if random.random() < 0.5:
            img = img.filter(ImageFilter.GaussianBlur(
                radius=random.random()))
            imgreverse = imgreverse.filter(ImageFilter.GaussianBlur(
                radius=random.random()))
        # final transform
        img, imgreverse, mask = np.array(img), np.array(imgreverse), np.array(mask, dtype=np.float32)
        return img, imgreverse, mask
    ...
    def __getitem__(self, idx):
        ...
        imgreverse_path = self.images_reverse+'/'+img_id+self.suffix
        ...
        imgreverse = Image.open(imgreverse_path).convert('RGB')
        mask = Image.open(label_path)

        # synchronized transform
        img, imgreverse, mask = self._sync_transform(img, imgreverse, mask)

        # general resize, normalize and toTensor
        if self.transform is not None:
            img = self.transform(img)
            imgreverse = self.transform(imgreverse)
        mask = np.expand_dims(mask, axis=0).astype('float32')/ 255.0


        return img, imgreverse ,torch.from_numpy(mask) #img_id[-1]
...
class TestSetLoader(Dataset):
    ...
    def _testval_sync_transform(self, img, img_reverse, mask):
        base_size = self.base_size
        img  = img.resize ((base_size, base_size), Image.BILINEAR)
        img_reverse = img_reverse.resize((base_size, base_size), Image.BILINEAR)
        mask = mask.resize((base_size, base_size), Image.NEAREST)

        # final transform
        img, img_reverse, mask = np.array(img), np.array(img_reverse), np.array(mask, dtype=np.float32)  # img: <class 'mxnet.ndarray.ndarray.NDArray'> (512, 512, 3)
        return img, img_reverse, mask

    def __getitem__(self, idx):
        # print('idx:',idx)
        img_id = self._items[idx]  # idx：('../SIRST', 'Misc_70') 成对出现，因为我的workers设置为了2
        img_path   = self.images+'/'+img_id+self.suffix    # img_id的数值正好补了self._image_path在上面定义的2个空
        img_reverse_path = self.images_reverse+'/'+img_id+self.suffix
        label_path = self.masks +'/'+img_id+self.suffix
        img  = Image.open(img_path).convert('RGB')  ##由于输入的三通道、单通道图像都有，所以统一转成RGB的三通道，这也符合Unet等网络的期待尺寸
        img_reverse = Image.open(img_reverse_path).convert('RGB')
        mask = Image.open(label_path)
        # synchronized transform
        img, img_reverse, mask = self._testval_sync_transform(img, img_reverse, mask)

        # general resize, normalize and toTensor
        if self.transform is not None:
            img = self.transform(img)
            img_reverse = self.transform(img_reverse)
        mask = np.expand_dims(mask, axis=0).astype('float32') / 255.0


        return img, img_reverse, torch.from_numpy(mask)  # img_id[-1]
...
def save_result_for_test(dataset_dir, st_model, epochs, best_iou, recall, precision ):
    with open(dataset_dir + '/' + 'roc_value_result'+'/' + st_model +'_best_IoU.log', 'a') as f:
        now = datetime.now()
        dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
        f.write('{} - {:04d}:\t{:.4f}\n'.format(dt_string, epochs, best_iou))

    with open(dataset_dir + '/' +'roc_value_result'+'/'+ st_model + '_best_other_metric.log', 'a') as f:
        f.write(dt_string)
        f.write('-')
        f.write(str(epochs))
        f.write('\n')
...
def save_result2_for_test(dataset_dir, st_model, epochs, best_iou, FA, PD ):
    with open(dataset_dir + '/' + 'roc_value_result'+'/' + st_model +'_best_nnnnIoU.log', 'a') as f:
        now = datetime.now()
        dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
        f.write('{} - {:04d}:\t{:.4f}\n'.format(dt_string, epochs, best_iou))

    with open(dataset_dir + '/' +'roc_value_result'+'/'+ st_model + '_best_FA_PD.log', 'a') as f:
        f.write(dt_string)
        f.write('-')
        f.write(str(epochs))
        f.write('\n')
